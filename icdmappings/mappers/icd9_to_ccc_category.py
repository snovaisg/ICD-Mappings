...
class ICD9toCCCCategory(MapperInterface):
    """
    Maps ICD-9 codes to CCC (Complex Chronic Conditions) categories.
    ...
    def map(self, code: Union[str, Iterable]) -> Union[str, Iterable]:
        """
        Given an ICD-9 code, returns the corresponding CCC category.
        ...
        Returns:
            CCC category or None when the mapping is not possible
        """
        if isinstance(code, str):
    ...
    def _parse_file(self, filename: str):
        ...
        with importlib_resources.files(pediatric_ccc).joinpath(filename).open() as csvfile:
            ...
            for row in reader:
                ...
                ccc_category = row[3]         # CCC_Category
                if icd_version == 9:          # Filter for ICD-9 only
                    mapping[icd_code] = ccc_category
        return mapping
