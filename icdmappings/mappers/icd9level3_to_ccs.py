...
class ICD9Level3toCCS(MapperInterface):
        ...
        def _setup(self):
            # {ccs:[icd9,...],..., ccs:[icd9,...]}
            self.ccs_to_icd9level3 = self._parse_file(self.filename)

            # (inverse mapping): {icd9:ccs,..., icd9:ccs}
            self.icd9level3_to_ccs = {self.ccs_to_icd9level3[ccs][i]:ccs for ccs in self.ccs_to_icd9level3 for i in range(len(self.ccs_to_icd9level3[ccs]))}
        ...
        def map(self,
            ...
            if isinstance(icd9level3code, str):
                return self._map_single(icd9level3code)
        ...
        def _get_codes(self, content : str):
            ...
            for group in groups:
                ...
                for tok in tokens:
                    ...
                    # only interested in the first 3 digits
                    tok_3 = tok[:3]
                    data[ccs_code].append(tok_3)
            return data